...
<body>
  <section id="box">
  	...
  	<span>{{search}}</span> <!-- v-model과 같은 이름일 경우 입력한 값을 바로 출력 -->
  	<br><br>
  ...
  <form id="f" method="get" action="./vue4.jsp">
  	  <!--  form 은 vue와 상관없이 사용가능 -->
	  <section id="box2">
	  <!-- checkbox, radio는 value가 적용 되어서 get, post로 값을 전송할 수 있음 -->
		<input type="checkbox" name="ag" value="Y" v-model="agree">동의함<br>
		 <!-- 실제 전송시에는 value 설정을 해야함  -->
		<input type="button" value="클릭" v-on:click="aaa"> <!--  vue 핸들링 -->
	  </section>
 </form>
	
</body>
<script src="./vue4.js?v=4"></script>
</html>
<!--  같은 name은 백엔드는 배열로 받는 -->
